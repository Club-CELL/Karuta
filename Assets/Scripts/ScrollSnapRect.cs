...
/// Simple script that allow you to scroll through a gallery of content with
/// the scrolling snapping on the content.
/// </summary>
...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    public void RefreshIndicators()
    {
        if(_pageSelectionImages != null)
        {
            ...
            {
                if (i == _previousPageSelectionIndex)
                {
                    _pageSelectionImages[i].color = selectedColor;
                    _pageSelectionImages[i].sprite = selectedPage;
                }
                else
                {
                    _pageSelectionImages[i].color = unselectedColor;
                    _pageSelectionImages[i].sprite = unselectedPage;
                }
            }
        }
    ...
    private void SetPageSelection(int aPageIndex) {
        ...
        Debug.Log(aPageIndex);
        if(themeLoaderMainMenu != null)
        {
            themeLoaderMainMenu.LoadThemeAtIndex(aPageIndex);
        }

        startingPage = aPageIndex;
